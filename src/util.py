    ...
    def bfs(self, starting_room_id, destination_room_id):
        ...
        while q.size() > 0:
            ...
            if room_id=="?":
                ...
                if room_id not in visited:
                    # print("room_id bfs in if statement",room_id)
                    if room_id == destination_room_id:
                        # print("INSIDE",path[1:])
                        return path
                    visited.add(room_id)
                    for next_room in self.get_neighbors(room_id):
                        # print("next_room in self.get_neighbors",next_room,self.directions[room_id][next_room])
                        new_path = list(path)
                        new_path.append(self.directions[room_id][next_room])
                        print("new_path WOOOO",new_path)
                        q.enqueue(new_path)
    def bfsAfter(self, starting_room_id, destination_room_id):
        # print("BFS starting_room_id, destination_room_id", starting_room_id, destination_room_id)
        q = Queue()
        q.enqueue([starting_room_id])
        visited = set()
